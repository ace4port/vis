...
const Sort = ({ wd, spd, type, arr, setTime }) => {
	const [width, setWidth] = useState(w[wd])
	const [speed, setSpeed] = useState(s[spd])
	...
	const draw = (p5) => {
		...
		for (let i = 0; i < values.length; i++) {
			clr[i] === 0 ? p5.fill(250, 250, 0) :
			clr[i] === 1 ? p5.fill(0, 255, 255) :
			clr[i] === 2 ? p5.fill(220, 0, 200) :
			clr[i] === 3 ? p5.fill(20, 200, 20) :
			p5.fill(255, 0, 255)
			p5.rect(i * w, p5.height - values[i], w, values[i])
		}
	}

	const bubble = async (arr, p5) => {
		for (let i = 0; i < arr.length - 1; i++) {
			...
			if (i === values.length - 2) {
				setQ(5)
				console.log(q, performance.now())
			}
		}
		p5.noLoop()
	}

	const insertion = async (arr, p5) => {
		for (let i = 1; i < arr.length; i++) {
			...
			clr[i] = 3
			if (i === values.length - 1) setQ(performance.now())
		}
		p5.noLoop()
	}
